...
function normalizeModelOutput(raw, requestedIntakeGrams = null) {
  ...
  // A model can occasionally follow a user's food name even after noticing
  // that the image is a screen capture. Treat any declared or described
  // screenshot medium as invalid instead of letting text override the image.
  if (looksLikeScreenCapture(raw, kind, modelNote)) {
    return {
      valid: false,
      rejectionMessage: "这似乎是截图或屏幕内容，请重新拍一张真实食物、包装或营养成分表。",
    };
  }
  ...
  return {
    ...
    data: {
      ...
      nutritionPerServing,
      // The web UI scales this base value with the editable intake slider.
      // Returning the already-scaled total here would double-count the ratio
      // whenever the user eats a different amount from one serving.
      riceEquivalent: {
