  ...
  const payload = {
    ...
    messages: [
      { role: "system", content: SYSTEM_PROMPT },
      {
        role: "user",
        content: [
          { type: "text", text: context ? `用户补充：${context}` : "用户没有补充说明。请基于图片做谨慎判断；如果是菜肴，明确标注估算。" },
...
function normalizeModelOutput(raw) {
  ...
  const scopeLabel = cleanText(firstPresent(sourceServing, ["scope_label", "scopeLabel", "intake_label", "intakeLabel"]), 120) || `按本次约 ${round(intakeGrams, 0)}g 估算`;
  return {
    valid: true,
    data: {
